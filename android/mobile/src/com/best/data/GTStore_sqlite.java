...
public class GTStore_sqlite{
    ...
    public static String[][] resultSetf,resultSetd,_finalres;
    ...
    public static String _currTime;
	...
	public static String[][] getAllStopsByText(String matchString, Double lat, Double lon)
	{
		...
		try{
			...
			if(lang==false)  //false means marathi
            {
			query =  "select g._stopID,_stopNameMarathi,_stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopNameMarathi like '"+matchString+"%' or _stopNameMarathi like '% "+matchString+"%' ) UNION select g._stopID, _stopNameMarathi, _stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopRoad like '"+matchString+"%' or _stopRoad like '% "+matchString+"%' ) UNION select g._stopID, _stopNameMarathi, _stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopArea like '"+matchString+"%' or _stopArea like '% "+matchString+"%' )  limit 100;";

			query1 = "select g._stopID, _stopNameMarathi,_stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopNameMarathi like '"+matchString+"%' or _stopNameMarathi like '% "+matchString+"%' ) limit 30;";

			query2 = "select g._stopID, _stopNameMarathi, _stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopRoad like '"+matchString+"%' or _stopRoad like '% "+matchString+"%' ) limit 30;";

		    query3 = "select g._stopID, _stopNameMarathi, _stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopArea like '"+matchString+"%' or _stopArea like '% "+matchString+"%' )  limit 30;";
            }
            else
            {
                query =  "select g._stopID,_stopName,_stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopName like '"+matchString+"%' or _stopName like '% "+matchString+"%' ) UNION select g._stopID, _stopName, _stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopRoad like '"+matchString+"%' or _stopRoad like '% "+matchString+"%' ) UNION select g._stopID, _stopName, _stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopArea like '"+matchString+"%' or _stopArea like '% "+matchString+"%' )  limit 100;";

                query1 = "select g._stopID, _stopName,_stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopName like '"+matchString+"%' or _stopName like '% "+matchString+"%' ) limit 30;";

                query2 = "select g._stopID, _stopName, _stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopRoad like '"+matchString+"%' or _stopRoad like '% "+matchString+"%' ) limit 30;";

                query3 = "select g._stopID, _stopName, _stopRoad, _stopArea from stop_names n, stop_geo g where n._stopID = g._stopID and ( _stopArea like '"+matchString+"%' or _stopArea like '% "+matchString+"%' )  limit 30;";
            }
	...
	public static String[][] getNearByStops(Double lat, Double lon)
	{
        ...
        String nearByStopQuery="";
        if(lang==false) //false means marathi
		 nearByStopQuery = "select g._stopID,_stopNameMarathi, _stopLat, _stopLon from stop_geo g, stop_names n where g._stopID = n._stopID and _stopLat between '"+minLatitude+"' and '"+maxLatitude+"' and _stopLon between '"+minLongitude+"' and '"+maxLongitude+"' limit 30 ;";
		else
            nearByStopQuery = "select g._stopID,_stopName, _stopLat, _stopLon from stop_geo g, stop_names n where g._stopID = n._stopID and _stopLat between '"+minLatitude+"' and '"+maxLatitude+"' and _stopLon between '"+minLongitude+"' and '"+maxLongitude+"' limit 30 ;";
	...
	{
        currentDateTime = Funcs.getCurrentDateTime();
	...
	{   String query="";
		if(lang==false) //false means marathi
        query = "select _searchID, _sourceStopID,(select _stopNameMarathi from stop_names where _stopID = _sourceStopID), _destStopID,(select _stopNameMarathi from stop_names where _stopID = _destStopID), _searchDate from previous_search;";
		else
            query = "select _searchID, _sourceStopID,(select _stopName from stop_names where _stopID = _sourceStopID), _destStopID,(select _stopName from stop_names where _stopID = _destStopID), _searchDate from previous_search;";
        return null ;
	}
	...
	public static String[][] getTripInfo(String tripID)
	{
        String q="";
		if(lang==false)
        q = "select distinct _stopNameMarathi, stop_names._stopID _stopID, _stopSeq ,_stopLat, _stopLon, _stopArea,_stopRoad from trips, stops_on_trip, stop_names, stop_geo where trips._tripID = '"+tripID+"' and trips._tripID = stops_on_trip._tripID and stops_on_trip._stopID = stop_names._stopID and stop_geo._stopID = stop_names._stopID order by _stopSeq asc;";
		else
        q = "select distinct _stopName, stop_names._stopID _stopID, _stopSeq ,_stopLat, _stopLon, _stopArea,_stopRoad from trips, stops_on_trip, stop_names, stop_geo where trips._tripID = '"+tripID+"' and trips._tripID = stops_on_trip._tripID and stops_on_trip._stopID = stop_names._stopID and stop_geo._stopID = stop_names._stopID order by _stopSeq asc;";
		System.out.println("==Query:getTripInfo=="+q);
        mode=false;
        inter.getMode(mode,null,null,null);
		...
		if( _allPassingRoutes != null )
		{
			...
			for( int cnt = 0; cnt < _allPassingRoutes.length; cnt++ )
			{
				...
				if( !_uniqPassingRoutes.containsKey( _uniqRouteId ) )
				{
					...
					_currTime = Funcs.getCurrentTime( true );
	...
	public static String[][] getDirectRoutes(String fromStopID, String toStopID)
	{
		...
		System.out.println("-----CurrentDAy----"+_currDay);//really called
        ...
        String qf = "select _stopID, _stopLat,_stopLon  from stop_geo where _stopID='"+fromStopID+"'; ";
        String qd = "select _stopID, _stopLat,_stopLon  from stop_geo where _stopID='"+toStopID+"'; ";
		Best.log( qf);
        Best.log( qd);
		resultSetf = dbhandle.execSelectQuery( qf,3);
        resultSetd = dbhandle.execSelectQuery( qd,3);
        currentDateTime = Funcs.getCurrentDateTime();

        format = new SimpleDateFormat("yyyy-MM-dd' 'HH:mm:ss");
        try {
            date = format.parse(currentDateTime);
            ...
            System.out.println(am_pmn+"am_pmn");
            dateTimeArray= new String[]{String.valueOf(hour),String.valueOf(minute),am_pmn,String.valueOf(DATE),String.valueOf(MONTH),String.valueOf(YEAR)};
        }
        ...
        //_currTime = Funcs.getCurrentTime( true );
		Best.log("===resultSetf.Length=="+resultSetf.length);
        Best.log("===resultSetd.Length=="+resultSetd.length);
        mode=true;
        _finalres=inter.getMode(mode,resultSetf,resultSetd,dateTimeArray );
        return( _finalres );
		/*if(resultSetf != null )
		{
			...
			String _allTripId = "";
			int _passedTrips = 0;
			for( int cnt = 0; cnt < resultSetf.length; cnt++ )
			{
				Best.log("====resultSet Count==="+ cnt);
				String _tripId = resultSetf[ cnt ][ 0 ];
				int _seq = Integer.parseInt( resultSetf[ cnt ][ 1 ] );
				String _startTime = resultSetf[ cnt ][ 2 ];
				String _endTime = resultSetf[ cnt ][ 3 ];
				int _freq = Integer.parseInt( resultSetf[ cnt ][ 5] );
				String _routeLongName = resultSetf[ cnt ][ 6 ];
				String _routeName = resultSetf[ cnt ][ 7 ];
			...
			if( _passingFromStopsTrips.size() > 0 && _allTripId.length() > 0 )
			{
                ...
                resultSetf = dbhandle.execSelectQuery( q2 ,2 );
				...
				if( resultSetf != null )
				{
					...
					for( int c = 0; c < resultSetf.length; c++ )
					{
						String tripId = resultSetf[ c ][ 0 ];
						int _toStopSeq = Integer.parseInt( resultSetf[ c ][ 1 ] );
	...
	{
		String _currDay = Funcs.getCurrentDay();
        String allStopsQuery="";
		if(lang==false)//false for marathi
        allStopsQuery = "select _stopID, _stopNameMarathi, _stopRoad, _stopArea from stop_names group by _stopNameMarathi order by _stopNameMarathi asc limit 100;";
        else
        allStopsQuery = "select _stopID, _stopName, _stopRoad, _stopArea from stop_names group by _stopName order by _stopName asc limit 100;";
        mode=false;
        inter.getMode(mode,null,null,null);
	...
	{
		String _currDay = Funcs.getCurrentDay();
		String allTripsQuery = "select distinct routes._routeID, trips._tripID, _routeLongName ,_routeName  from routes, trips, schedule_rules where trips._routeID = routes._routeID   and trips._serviceID = schedule_rules._serviceID and _serviceDay = '"+_currDay+"' group by routes._routeID order by routes._routeID asc ;";
        mode=true;
        inter.getMode(mode,null,null,null);
        System.out.println("allTrips QUERY==:"+allTripsQuery);
		String[][] allTrips = dbhandle.execSelectQuery( allTripsQuery , 4);
		return (allTrips);
	}
	...
	{
		String _currDay = Funcs.getCurrentDay();
		String allTripsByTextQuery = "select distinct routes._routeID, trips._tripID, _routeLongName ,_routeName  from routes, trips, schedule_rules where ( _routeLongName like '"+matchText+"%' or _routeLongName like '% "+matchText+"%' or  _routeName like '"+matchText+"%' or _routeName like '% "+matchText+"%') and trips._routeID = routes._routeID  and trips._serviceID = schedule_rules._serviceID  and _serviceDay = '"+_currDay+"' group by routes._routeID order by routes._routeID asc limit 100;";
		
		System.out.println("allTrips QUERY==:"+allTripsByTextQuery);
		String[][] allTrips = dbhandle.execSelectQuery( allTripsByTextQuery , 4);
		return (allTrips);
	}
	...
	{
		String _currDay = Funcs.getCurrentDay();
		String query  = " select distinct _stopID, routes._routeID, trips._tripID, _routeLongName ,_routeName  from stops_on_trip, routes,trips where trips._tripID = stops_on_trip._tripID and _stopID = '"+stopID+"' and trips._routeID = routes._routeID  group by routes._routeID order by routes._routeID asc limit 30;";
        mode=false;
        inter.getMode(mode,null,null,null);
		String[][] allPassingByTrips = dbhandle.execSelectQuery( query , 5);
	...
	public static String[][] getNearByTripInfo(Double lat, Double lon)
	{
		...
		String nearByPassingTrips = "select distinct routes._routeID, trips._tripID, _routeLongName ,_routeName  from routes, trips, schedule_rules where trips._routeID = routes._routeID  and trips._tripID in (select _tripID from stops_on_trip where _stopID in ( select stop_geo._stopID from stop_geo where _stopLat between '"+minLatitude+"' and '"+maxLatitude+"' and _stopLon between '"+minLongitude+"' and '"+maxLongitude+"' limit 30))  and trips._serviceID = schedule_rules._serviceID and _serviceDay = '"+_currDay+"' group by routes._routeID order by routes._routeID asc ;";
		
		System.out.println("QUERY==:"+nearByPassingTrips);
        mode=false;
        inter.getMode(mode,null,null,null);
		String[][] nearbyTrips_Name = dbhandle.execSelectQuery( nearByPassingTrips , 4);
